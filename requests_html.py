...
class AsyncHTMLSession(BaseSession):
    ...
    def run(self, *coros, progress=True):
        ...
        if progress:
            tasks.append(asyncio.ensure_future(self.progress(tasks.copy())))

        done, _ = self.loop.run_until_complete(asyncio.wait(tasks))
        
        return [t.result() for t in done if t.result()!=None]
    
    
    async def progress(self, tasks):
        totalTasks = len([task for task in tasks if not task.done()])
        while(True):
            await asyncio.sleep(0.1)
            pending = len([task for task in tasks if not task.done()])
            self.progressBar(totalTasks,totalTasks - pending)
            if (pending) <=0:
                break

    
    def progressBar(self, globalLength, iterazioneGlobale, lunghezzaBarraProgressoGlobale = 50):
        
        # Progresso globale
        perc = int(((iterazioneGlobale)/globalLength)*10000)/100
        progress = int(((iterazioneGlobale)/globalLength)*lunghezzaBarraProgressoGlobale)
